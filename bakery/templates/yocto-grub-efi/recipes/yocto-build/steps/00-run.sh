...
mkdir -p "${RUGIX_LAYER_DIR}/yocto/images/"
cp -r "${KAS_BUILD_DIR}/tmp/deploy/images/"* "${RUGIX_LAYER_DIR}/yocto/images/"

# Extract the root filesystem into a location where Rugix expects it.
mkdir -p "${RUGIX_ROOT_DIR}"
ARCHIVE=$(echo "${RUGIX_LAYER_DIR}/yocto/images/"*"/core-image-base-"@(qemuarm64|qemux86-64)".tar.bz2")
