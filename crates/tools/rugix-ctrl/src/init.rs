...
use rugix_common::ctrl_config::{load_config, Config, Overlay, CTRL_CONFIG_PATH};
...
use rugix_common::system::config::{load_system_config, PartitionConfig};
use rugix_common::system::partitions::{resolve_config_partition, resolve_data_partition};
use rugix_common::system::paths::{MOUNT_POINT_CONFIG, MOUNT_POINT_DATA, MOUNT_POINT_SYSTEM};
use rugix_common::system::root::{find_system_device, SystemRoot};
use rugix_common::system::slots::SlotKind;
use rugix_common::system::{System, SystemError, SystemResult};
...
fn init() -> SystemResult<()> {
    ...
    let Some(config_partition) =
        ...
        bail!("Rugix pre-init requires a config partition");
    };
