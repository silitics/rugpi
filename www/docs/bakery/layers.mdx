...
In addition to project-specific layers and recipes defined in the project directory, you can use layers and recipes provided by [repositories](./repositories.mdx). In case of the example, these repositories are `core` and `rugix-extra`. We will explain them in detail later. The `core` repository is shipped with Rugix Bakery and includes the following layers for you to use as a parent layer:
