...
```shell
rugix-ctrl update install <path to the image or bundle>
```
...
```shell
curl <url to the image> | rugix-ctrl update install -
```
...
To prevent manipulated updates from being installed, Rugix Ctrl provides the option `--check-hash` which can be used to check the SHA256 hash of a (streamed) update.
Rugix Ctrl does not implement its own signature mechanism yet.
While we are working towards cryptographic verification of updates directly in Rugix Ctrl, so far, this is up to the delivery mechanism.
Recall that Rugix Ctrl is only an update installer and does not prescribe how the updates get on the device.
For now, we push the responsibility of verifying signatures to the delivery mechanism.
For instance, you can use GPG or OpenSSL to sign an _update manifest_ that contains an URL and the SHA256 hash of the update.
In a typical setup, the delivery mechanism would then first check the signature of the manifest and then install the update by feeding it via `wget` or `curl` (see above) into Rugix Ctrl, providing the hash via the `--check-hash` option.
This ensures that Rugix Ctrl will check the hash and abort the update if the hash does not match.

Here is an example with the `--check-hash` option:

```shell
rugix-ctrl update install --check-hash sha256:a9627e22da964b5b6ad7c1465a79bae4d11b71a064966b37596c057de106c1a9 <path to the image or bundle>
```

Note that this mechanism is very flexible and allows us to build on trusted tools in a simple way instead of implementing our own signature verification, which may introduce security vulnerabilities.
You can also combine an update and the signed manifest in a `.tar` archive and process that in a streaming fashion as part of your update workflow.

:::warning
There is one downside to this approach: The hash can be checked only after the update has been written to disk.
This means that manipulated data may have been written to the boot and system partition where the update is installed.
You must be careful not to initiate a rollback into that system, thereby accidentally booting a manipulated system.
When Rugix Ctrl finds that the hash does not match, it will partially overwrite those partitions, however, an attacker with physical access may prevent that by cutting power at the right time and may then somehow exploit your application to do a rollback.
In the future, Rugix Ctrl will provide a mechanism based on a signed block list with per-block hashes.
It will then verify the integrity of each block **before** writing it to disk, thereby ensuring that potentially manipulated data never finds their way onto the disk.
:::
