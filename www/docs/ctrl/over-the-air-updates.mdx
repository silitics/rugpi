...
- As the previous version is kept, a rollback to the old version is possible if there are any problems with the new version.[^rollback]
...
As a reference, here is the full schema of the `rugix-ctrl system info` output:
...
An update might be installed from an USB stick (local file), uploaded via a web UI (streaming via stdin), or downloaded from an update server (HTTP).
...
Unless the bundle is provided on an external storage medium, like a USB stick, we recommend that you stream updates to devices.
Streaming updates are not only faster, since the update has to be written to storage only once, but also have the advantage that they do not take up precious space on the data partition during the installation.
If your device is running low on storage space, non-streaming updates may become impossible, leading to failed updates.
...
[^delta-read-only]: Currently, this requires the system partition to be read-only. We will lift that restriction in the future. However, note that a read-only system partition is recommended in any case and made easy with Rugix Ctrl's state management functionality.
...
If you want to use adaptive delta updates right now, just create the index before installing the update bundle via HTTP.
...
To prevent automatic reboots, use the `--no-reboot` command line flag:

```shell
rugix-ctrl update install --no-reboot <update bundle>
```
...
However, you can still implement signed updates in multiple ways on top of Rugix Ctrl's flexible streaming functionality.
