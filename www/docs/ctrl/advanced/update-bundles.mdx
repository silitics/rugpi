...
A payload can have different types, configured in the `config` section.

#### `type = "slot"`

Payload is going to be installed to the specified slot.

#### `type = "script"`

Payload is an _update script_.
Update scripts are written to a temporary directory and then executed.


## Delta Updates

:::warning
**Delta updates are not yet implemented.** This section describes what we plan to implement.
:::

### Adaptive Delta Updates

Adaptive delta updates with any HTTP server supporting range queries, making them particularly easy to implement.
