...
These considerations give rise to the following pipeline:

```mermaid
flowchart LR
    slot_data[Slot Data] -- preprocess --> payload_file[Payload File]
    payload_file -- pack --> update_bundle[Update Bundle]
    payload_file -- install --> slot_data
...
Note that Rugix Ctrl implements streaming updates, i.e., the unpacking step will not reconstruct the entire payload file but instead stream it to the installation step in pieces as the update bundle is read.

## Packing and Unpacking

To pack and unpack bundles, you can use Rugix Bundler (`rugix-bundler`).
...
Rugix Bundler offers a subcommand for unpacking and packing a bundle, respectively:

```shell
rugix-bundler unpack <bundle path.rugixb> <bundle directory>
rugix-bundler pack <bundle directory> <bundle path.rugixb>
```

The _bundle directory_ is a directory with the following structure:
...
chunker = { type = "fixed", block-size = "32KiB" }
deduplication = false
compression = { type = "xz" }
...
:::warning
**Delta updates are not yet implemented.** The following describes how we will implement them.
:::

### Adaptive Delta Updates

Adaptive delta updates are easy to implement and work with any HTTP server supporting range queries.
...
In addition to adaptive delta updates, we will also implement static delta updates, where you create a bundle to specifically go from one version to another.
...
To this end, the payload file will first be divided into blocks (the examples above use a fixed block size of `32KiB`).
...
When unpacking the payload, the block index is used to verify the integrity of each block individually, thereby establishing trust in their contents before thy are installed/written anywhere.
...
**Block Deduplication.** 
Normally, we would write the payload file directory into the bundle.
...
First, the payload file must be written directly to the slot without any post processing.
Second, we need random access on the slot.
For these reasons, block deduplication is incompatible with payloads that have have a postprocessing step.
For instance, for directory slots, the payload file is a `.tar` archive that is extracted on-the-fly to the respective directory.
In this case, the extraction of the archive is a postprocessing step.
The same applies to static deltas where the payload file is fed through a delta compression tool (like `xdelta`) whose output is then written to the respective slot.

**Adaptive Delta Updates.**
We can use the block index to adaptively only fetch those blocks of the payload data that we do not already have locally on the device.
This requires the ability to efficiently skip over blocks in the payload data, that we do already have.
Skipping can be realized by HTTP range queries.
...
In case of variable block sizes, e.g., when using a rolling hash to divide the payload file (as done by [Casync](https://github.com/systemd/casync)), we need an additional sequence storing the block sizes called a _size index_.
For this sequence, we can always use block deduplication, assuming that blocks with an identical hash also have an identical size.

**Block Compression.**
To reduce the size of the update bundle while still enabling adaptive delta updates and block-wise cryptographic verification of updates, we can compress blocks individually.
